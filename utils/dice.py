...
class NNRecordWrapper:
    ...
    def predict(self, x):
        ...
        cf_input = self.dice_to_input(x)
        return (
            (self.model(tf.constant(cf_input.astype(float)), training=False) > 0.5)
            .numpy()
            .astype(int)[0]
        )
    ...
    def predict_proba(self, x):
        ...
        model_output = self.model(
            tf.constant(cf_input.astype(float)), training=False
        ).numpy()
...
def get_dice_cfs(data_interface, wrapped_models):
    ...
    return {
        "dt": dice_ml.Dice(
            data_interface, dice_ml.Model(model=wrapped_models["dt"], backend="sklearn")
        ),
        "rfc": dice_ml.Dice(
            data_interface,
            dice_ml.Model(model=wrapped_models["rfc"], backend="sklearn"),
        ),
        "nn": dice_ml.Dice(
            data_interface, dice_ml.Model(model=wrapped_models["nn"], backend="sklearn")
        ),
    }
...
def generate_dice_result(
    ...
    d = dice_ml.Data(
        dataframe=df_info.scaled_df,
        continuous_features=df_info.numerical_cols,
        outcome_name=df_info.target_name,
    )
    ...
    for k in models_to_run:
        ...
        for idx, instance in enumerate(
            df_info.scaled_df.iloc[
                test_df[test_start_idx : test_start_idx + num_instances].index
            ].iloc
        ):
            ...
            for num_cf in range(num_cf_per_instance):
                ...
                prediction = df_info.target_label_encoder.inverse_transform(
                    wrapped_models[k].predict(input_query)
                )[0]
        ...
        for i in range(len(results[k])):
            ...
            if not results[k][i]["cf"] is None:
                ...
                scaled_cf_df.loc[
                    ...
                    [scaled_cf_df.loc[0, df_info.target_name]]
                )[
                    0
                ]
