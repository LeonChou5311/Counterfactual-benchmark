...
def get_evaluation(df, feature_values, feature_names, surrogate_wrapper, scaler, generator):
    ...
    result_df = pd.DataFrame({}, columns=['input_query', 'cf', 'input_query_nn_pred', 'cf_nn_pred', 'permutation_nn_pred_portion', 'input_query_tree_pred', 'cf_tree_pred', 'input_query_decisions', 'cf_decisions', 'dot_data', 'sparsity', 'distance', 'depth_diff', 'has_same_input_pred', 'has_same_cf_pred'])
    ...
    for idx in range(len(df)):
        ...
        original_nn_pred = surrogate_wrapper.predict(scaler.transform(origin)).numpy()[0][0]
        cf_nn_pred = surrogate_wrapper.predict(scaler.transform(cf)).numpy()[0][0]
        ...
        origin_decisions, origin_tree_pred = rules.get_decision_process(origin, tree, feature_names)

        cf_decisions, cf_tree_pred = rules.get_decision_process(cf, tree, feature_names)
